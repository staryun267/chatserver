...
int main(int argc,char **argv)
{
    ...
    for(;;)
    {
        ...
        case 1:     //login业务
        {
            ...
            int len = send(clientfd,response.c_str(),strlen(response.c_str())+1,0);
            if(-1 == len)
            {
                cerr << "send login msg error" << response << endl;
            }

            sem_wait(&rwsem);   //等待登录结果，子线程处理完登录响应后会通知这里
            if(g_isLoginSuccess)
            {
                //进入聊天主菜单页面
        ...
        case 2:     //register业务
        {
            ...
            sem_wait(&rwsem);   //等待注册结果，子线程处理完注册响应后会通知这里
        }
        break;
        case 3:     //quit业务
            close(clientfd);
    ...
    else // 登录成功
    {
        ...
        if (responsejs.contains("offlinemsg"))
        {
            ...
            for (string &str : vec)
            {
                ...
                {
                    cout << js["time"] << " [" << js["id"] << "] " << js["name"] << " said: " << js["msg"].get<string>() << endl;
                }
                else
                {
...
void readTaskHandler(int clientfd)
{
    for(;;)
    {
        ...
        {
            doLoginResponse(js);
            sem_post(&rwsem);   //通知main线程，登录结果处理完成
            continue;
        }
        ...
        {
            doRegResponse(js);
            sem_post(&rwsem);   //通知main线程，注册结果处理完成
            continue;
        }
    }
}
...
void mainMenu(int clientfd)
{
    ...
    while(isMainMenuRunning)
    {
        ...
        if(idx == -1)
        {
            command = commandbuf;   //help或loginout
        }
        else
        {
        ...
        //调用相应命令的事件回调处理，mainNew对事件修改封闭，添加新功能不需要修改该函数
...
void creategroup(int clientfd,string str)
{
    ...
    if(-1 == idx)
    {
        cerr << "creategroup command error" << endl;
        return;
    }
